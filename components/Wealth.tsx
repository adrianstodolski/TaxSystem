...
import { TrendingUp, ArrowUpRight, ArrowDownRight, Activity, Plus } from 'lucide-react';
    ...
    const columns = useMemo<ColumnDef<StockAsset>[]>(() => [
        {
            ...
            cell: info => (
                <div className="flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold text-white border ${
                        info.row.original.type === 'STOCK' ? 'bg-indigo-600/20 border-indigo-500/30 text-indigo-400' :
                        info.row.original.type === 'ETF' ? 'bg-emerald-600/20 border-emerald-500/30 text-emerald-400' : 'bg-amber-500/20 border-amber-500/30 text-amber-400'
                    }`}>
        ...
        { accessorKey: 'name', header: 'Nazwa', cell: info => <span className="text-zinc-300">{info.getValue() as string}</span> },
        { accessorKey: 'quantity', header: 'Ilość', cell: info => <span className="font-mono text-zinc-400">{info.getValue() as number}</span> },
        { accessorKey: 'avgPrice', header: 'Cena Zakupu', cell: info => <span className="font-mono text-zinc-500">{info.getValue() as number} {info.row.original.currency}</span> },
            ...
            <header className="flex justify-between items-center border-b border-white/10 pb-6">
                <div>
                    <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                        <TrendingUp className="text-gold" /> Wealth Management
                    </h2>
                    <p className="text-zinc-400 mt-1">Globalny portfel inwestycyjny (Akcje, ETF, Surowce).</p>
                </div>
                <div className="flex gap-2">
                    <span className="text-xs font-bold text-zinc-400 bg-white/5 border border-white/10 px-3 py-1.5 rounded flex items-center gap-2">
                        <Activity size={14} className="text-green-500 animate-pulse" /> Market Open
                    </span>
                    <button onClick={() => toast.info('Integracja', 'Łączenie z XTB...')} className="bg-gold text-black px-4 py-2 rounded-xl font-bold hover:bg-[#FCD34D] flex items-center gap-2 text-sm shadow-lg shadow-gold/20">
                        <Plus size={16} /> Połącz Brokera
                    </button>
                </div>
            </header>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="neo-card text-white p-8 rounded-2xl shadow-2xl relative overflow-hidden bg-gradient-to-r from-onyx to-[#141419]">
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-12">
                    <div>
                        <p className="text-gold font-bold uppercase text-xs tracking-wider mb-2">Wartość Portfela</p>
