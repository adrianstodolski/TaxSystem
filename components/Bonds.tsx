    ...
    const columns = useMemo<ColumnDef<BondPosition>[]>(() => [
        {
            ...
            cell: info => (
                <div>
                    ...
                    <p className="text-xs text-slate-500 font-mono">{info.row.original.isin}</p>
                </div>
            )
        },
        {
        ...
        { accessorKey: 'faceValue', header: 'Nominał', cell: info => <span className="font-mono text-slate-400">{safeFormatCurrency(info.getValue() as number)}</span> },
        { accessorKey: 'quantity', header: 'Ilość', cell: info => <span className="font-mono">{info.getValue() as number}</span> },
        { accessorKey: 'couponRate', header: 'Kupon', cell: info => <span className="font-bold text-green-400">{info.getValue() as number}%</span> },
        { accessorKey: 'maturityDate', header: 'Wykup', cell: info => <span className="text-slate-500">{info.getValue() as string}</span> },
        { 
            accessorKey: 'inflationIndexed', 
            header: 'Indeksacja', 
            cell: info => info.getValue() ? <span className="inline-flex items-center gap-1 text-green-400 font-bold text-xs bg-green-500/20 px-2 py-1 rounded-full border border-green-500/30"><TrendingUp size={12} /> CPI</span> : <span className="text-xs text-slate-500">-</span> 
        }
    ], []);
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            <header className="flex justify-between items-center border-b border-white/10 pb-6">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">Portfel dłużny, obligacje skarbowe (EDO/COI) i korporacyjne (Catalyst).</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl border border-white/10">
                        ...
                        <div className="mt-4 flex items-center gap-2 text-sm"><span className="bg-green-500/20 px-2 py-1 rounded text-green-400 font-bold border border-green-500/30">YTM: {avgYield.toFixed(2)}%</span><span className="text-slate-400">Średnia rentowność</span></div>
                    </div>
                    <div className="glass-card p-6 rounded-2xl">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-sm"><span className="text-slate-400">Inflacja (CPI)</span><span className="font-bold text-white">8.2%</span></div>
                            <div className="flex justify-between text-sm"><span className="text-slate-400">Oprocentowanie EDO</span><span className="font-bold text-indigo-400">7.25% + Marża</span></div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">Twoje portfolio pokrywa 90% inflacji.</p>
                        </div>
                    </div>
                </div>
                <div className="lg:col-span-2 glass-card p-6 rounded-2xl flex flex-col">
                    <h3 className="font-bold text-white mb-6 flex items-center gap-2"><BarChart3 className="text-slate-400" /> Drabina Odsetkowa (Cashflow)</h3>
